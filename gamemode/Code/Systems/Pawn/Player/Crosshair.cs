...
public partial class Crosshair : Component
{
	...
	protected override void OnUpdate()
	{
		...
		if ( player.IsValid() && player.CameraController.IsValid() )
		{
			...
			if ( equipment.IsValid() )
			{
				...
				if ( !equipment.UseCrosshair )
				{
					if ( player.PickupProgress > 0f )
						DrawPickupProgress( hud, center, scale, player.PickupProgress );
					return;
				}
		...
		// Cercle de progression pour le ramassage (Hold E)
		if ( player.IsValid() && player.PickupProgress > 0f )
		{
			DrawPickupProgress( hud, center, scale, player.PickupProgress );
		}
	...
	private void DrawPickupProgress( Sandbox.Rendering.HudPainter hud, Vector2 center, float scale, float progress )
	{
		...
		for ( int i = 0; i < segments; i++ )
		{
			...
			if ( t0 >= angleMax ) break;
			if ( t1 > angleMax ) t1 = angleMax;

			var p0 = center + new Vector2( MathF.Sin( t0 ), -MathF.Cos( t0 ) ) * radius;
			var p1 = center + new Vector2( MathF.Sin( t1 ), -MathF.Cos( t1 ) ) * radius;
