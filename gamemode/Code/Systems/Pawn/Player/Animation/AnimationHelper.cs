...
public sealed class AnimationHelper : Component
{
	...
	[Rpc.Broadcast]
	public void CuffedAnimation()
	{
		if ( IkLeftHand.IsValid() && IkLeftHand.Active )
		{
			SetIk( "hand_left", IkLeftHand.Transform.World );
		}
		else ClearIk( "hand_left" );



		if ( IkRightHand.IsValid() && IkRightHand.Active ) {
			SetIk( "hand_right", IkRightHand.Transform.World );
			SetIk( "hand_right", IkRightHand.Transform.World );
		}
		else ClearIk( "hand_right" );
