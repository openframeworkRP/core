	...
	IGameEventHandler<EquipmentHolsteredEvent>
{
	...
	/// <summary>
	/// Backing byte synchronisé : BytePack ne sait pas sérialiser l'enum FireMode sur dédié,
	/// on passe donc par un byte et on expose l'enum via la propriété ci-dessous.
	/// </summary>
	[Sync, Group( "Fire Modes" )] public byte CurrentFireModeRaw { get; set; } = (byte)FireMode.Automatic;
	...
	{
		get => (FireMode)CurrentFireModeRaw;
		set => CurrentFireModeRaw = (byte)value;
	}
	...
	/// </summary>
	protected Ray WeaponRay => new Ray( Effector.Muzzle.WorldPosition, Effector.Muzzle.WorldRotation.Forward );
	...
	protected void DoShootEffects()
	{
		...
		if ( ShootSound is not null )
		{
			if ( ShootSound is not null )
			{
				// 1. On détermine le volume de base
				float finalVolume = 1.0f;

				// 2. On vérifie si le propriétaire de l'arme est dans une pièce
				// On utilise Equipment.Owner qui est le PlayerPawn
				if ( Equipment.Owner.IsValid() )
				{
					var currentRoom = AudioRoomBlocker.All.FirstOrDefault( x => x.PlayersInRoom.Contains( Equipment.Owner.GameObject ) );

					if ( currentRoom != null )
					{
						// On est à l'intérieur : on divise le volume par 2
						finalVolume = 0.7f;
						Log.Info( $"Tir en intérieur ({currentRoom.RoomName}) : Volume réduit." );
					}
				}

				// 3. On joue le son avec le volume ajusté
				if ( Sound.Play( ShootSound, Equipment.WorldPosition ) is { } snd )
				{
					snd.Volume *= finalVolume; // Application de la réduction
	...
	private void InflictDamage( GameObject target, float damage, Vector3 pos, Vector3 dir, HitboxTags hitbox, DamageFlags flags )
	{
		if ( target.IsValid() )
		{
			target.TakeDamage( new Pawn.DamageInfo( Equipment.Owner, damage, Equipment, pos, dir * damage, hitbox, flags ) );
		}
	}
