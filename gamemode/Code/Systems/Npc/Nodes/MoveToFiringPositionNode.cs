...
public class MoveToFiringPositionNode : BaseBehaviorNode
{
	...
	protected override NodeResult OnEvaluate( NpcContext context )
	{
		...
		var target = context.GetData<Pawn.Pawn>( CURRENT_TARGET_KEY );
	...
	private bool IsTargetVisibleFromPerception( NpcContext context, Pawn.Pawn target )
	{
		if ( !context.HasData( "visible_enemies" ) )
			return false;

		var visibleEnemies = context.GetData<List<Pawn.Pawn>>( "visible_enemies" );
